...
function ExchangeProducts(props) {
    ...
    const RenderProductsToExchange = ({ item, index }) => {
        const { description, isLiked, media, productName, _id, price, user } = item
        // console.log(index)
        return (
            <TouchableOpacity
    ...
    function RenderEmptyListComponent() {
        return (
            <View style={{ paddingTop: 100, justifyContent: 'center', alignItems: 'center', borderColor: 'red', borderWidth: 0 }}>
                <Text style={{ color: textOffColor, fontSize: 18 }}>No Exchange products to show!</Text>
            </View>
        )
    }
    useEffect(() => {
        ...
        <>
            <FlatList
                refreshControl={
                    <RefreshControl
                        refreshing={false}
                        onRefresh={() => GetExchangeProducts()}
                    />
                }
                data={productsToExchange}
                renderItem={RenderProductsToExchange}
